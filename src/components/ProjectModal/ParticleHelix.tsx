...
export default function ParticleMorphing({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const r = Math.cos(p * t) * 0.4 + 1.2; // 0.5+1.5 → 0.4+1.2に縮小

      const x = r * Math.cos(q * t);
      const y = r * Math.sin(q * t);
      const z = Math.sin(p * t) * 0.4; // 0.5 → 0.4に縮小

      // ランダムなオフセットで太さを出す（さらに太く）
      const offset = 0.35; // 0.15 → 0.35に増やす
      const randomX = x + (Math.random() - 0.5) * offset;
      const randomY = y + (Math.random() - 0.5) * offset;
      const randomZ = z + (Math.random() - 0.5) * offset;

      torusKnotPositions[i * 3] = randomX;
      torusKnotPositions[i * 3 + 1] = randomY;
      torusKnotPositions[i * 3 + 2] = randomZ;
    }

    // 螺旋の座標生成
    const helixPositions = new Float32Array(particleCount * 3);
    const radius = 0.7; // 0.8 → 0.7に縮小
    const height = 3.5; // 4 → 3.5に縮小
    const turns = 5;
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const spread = 0.4; // 0.2 → 0.4に増やして図太く
